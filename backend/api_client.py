    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if e.response.status_code == 429 and attempt < max_retries:
                print(f"Rate limit hit for {e.request.url if e.request else url}. Waiting 60 seconds to retry (Attempt {attempt + 1}/{max_retries + 1})...")
                time.sleep(60)
                continue
            else:
                print(f"HTTP error occurred for get_teams(league_code={league_code}): {e}")
            ...
            print(f"Failed to decode JSON response for get_teams(league_code={league_code}): {e}")
            return []
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if e.response.status_code == 429 and attempt < max_retries:
                ...
                print(f"HTTP error occurred for get_matches_for_team(team_id={team_id}, from={date_from}, to={date_to}): {e}")
                if hasattr(e, 'response') and e.response is not None:
                    print(f"Response status: {e.response.status_code}, Response content: {e.response.text}")
                return []
        except requests.exceptions.RequestException as e:
            ...
            print(f"Failed to decode JSON response for get_matches_for_team(team_id={team_id}, from={date_from}, to={date_to}): {e}")
            return []
        except Exception as e:
...
def get_head_to_head_matches(match_id: int) -> list:
    ...
    url = f"{API_BASE_URL}matches/{match_id}/head2head"
    max_retries = 1
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if result_set and result_set.get("count", 0) > 0 and matches_list:
                ...
                print(f"No h2h matches found for match_id {match_id}, 'resultSet' missing or 'matches' list empty. API Response: {data}")
                return []
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 429 and attempt < max_retries:
                print(f"Rate limit hit for {e.request.url if e.request else url}. Waiting 60 seconds to retry (Attempt {attempt + 1}/{max_retries + 1})...")
                ...
                print(f"HTTP error occurred for get_head_to_head_matches(match_id={match_id}): {e}")
                if hasattr(e, 'response') and e.response is not None:
                    print(f"Response status: {e.response.status_code}, Response content: {e.response.text}")
                return []
        except requests.exceptions.RequestException as e:
            ...
            print(f"Failed to decode JSON response for get_head_to_head_matches(match_id={match_id}): {e}")
            return []
        except Exception as e:
...
def get_league_standings(league_code: str) -> list:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            print(f"Failed to decode JSON response for get_league_standings(league_code={league_code}): {e}")
            return []
        except Exception as e:
